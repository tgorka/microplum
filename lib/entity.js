...
class SeedEntity extends ServiceEntity {
    addServices(seneca, options) {
        if (this.emptyFacade.reset) {
            seneca.add(this.pin(this.name, "reset", { conditions: "*" }), async (args) => this.createFacade(args).reset(args.seed));
        }
        if (this.emptyFacade.seed) {
            seneca.add(this.pin(this.name, "seed", { conditions: "*" }), async (args) => this.createFacade(args).seed());
        }
    }
}
